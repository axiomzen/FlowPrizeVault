...
access(all) contract MockYieldConnector {
    ...
    /// LimitedCapacityConnector - A connector with a configurable capacity limit
    ...
    access(all) struct LimitedCapacityConnector: DeFiActions.Sink, DeFiActions.Source {
        ...
        /// Returns an estimate of how much can be deposited (limited by capacityLimit)
        access(all) fun minimumCapacity(): UFix64 {
            if let provider = self.providerCap.borrow() {
                let currentBalance = provider.balance
                if currentBalance >= self.capacityLimit {
                    return 0.0  // Already at or over capacity
                }
                return self.capacityLimit - currentBalance
            }
            return self.capacityLimit  // If can't check balance, return full capacity
        }
        ...
        access(all) fun depositCapacity(from: auth(FungibleToken.Withdraw) &{FungibleToken.Vault}) {
            if let receiver = self.receiverCap.borrow() {
                ...
                // Only deposit up to available capacity
                if amount > 0.0 && availableCapacity > 0.0 {
                    let depositAmount = amount < availableCapacity ? amount : availableCapacity
    ...
    /// Creates a new LimitedCapacityConnector with a configurable capacity limit
