...
access(all) contract PrizeLinkedAccounts {    
    ...
    access(all) resource Pool {
        ...
        access(contract) fun deposit(from: @{FungibleToken.Vault}, receiverID: UInt64, ownerAddress: Address?, maxSlippageBps: UInt64) {
            ...
            self.allocatedRewards = self.allocatedRewards + actualReceived
        ...
        access(contract) fun sponsorDeposit(from: @{FungibleToken.Vault}, receiverID: UInt64, ownerAddress: Address?, maxSlippageBps: UInt64) {
            ...
            // Update pool total with ACTUAL amount received
            self.allocatedRewards = self.allocatedRewards + actualReceived
        ...
        access(contract) fun completeDraw() {
            ...
            for i in InclusiveRange(0, distributedWinners.length - 1) {
                ...
                // Update pool total (move from prize allocation to user balance)
