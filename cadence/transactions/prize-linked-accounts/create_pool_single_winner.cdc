...
/// Create Pool (Single Winner) - Creates a pool where one winner takes the entire prize
///
...
/// - rewardsPercent: Percentage of yield to savings interest (UFix64, e.g., 0.5 = 50%)
/// - prizePercent: Percentage of yield to prize pool (UFix64, e.g., 0.4 = 40%)
/// - protocolFeePercent: Percentage of yield to protocol treasury (UFix64, e.g., 0.1 = 10%)
/// - yieldVaultPath: Storage path identifier for yield vault (String, e.g., "testYieldVault")
transaction(
    ...
    execute {
        ...
        log("Created SingleWinner pool with ID: ".concat(poolID.toString()))
    }
}
